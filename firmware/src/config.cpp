...
bool command_init() {
  ...
  if (!LittleFS.exists("/config")) {
    ...
    const char default_config[30] = {
        ...
        0x48,  // 默认触摸对应按键，keyboard 工作模式及 keyboard with led
               // 工作模式有效
        0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F,
        0x50,  // 默认触摸对应按键，keyboard 工作模式及 keyboard with led
               // 工作模式有效
        0x30, 0x31, 0x32, 0x33, 0x34, 0x35,  // 默认天键对应按键，keyboard
                                             // 工作模式及 keyboard with led
                                             // 工作模式有效
        0x7F, 0x00, 0x23,  // 默认触摸区按下颜色，keyboard with led 工作模式有效
        0x00, 0x23, 0x7F  // 默认触摸区间隔颜色，keyboard with led 工作模式有效
    };
    size_t bytes_write = config_file.write(default_config);
    config_file.close();
    if (bytes_write != sizeof(default_config)) {
  ...
  size_t bytes_read = config_file.readBytes((char *)&config, sizeof(config));
  config_file.close();
  if (bytes_read != sizeof(config)) return false;
...
bool command_read() {
  ...
  // 获取完整指令
  if (Serial.readBytes((uint8_t *)&command_rx, sizeof(command_rx)) !=
      sizeof(command_rx))
    return false;
...
bool command_execute() {
  ...
  if (config_file.readBytes((char *)(&command_tx + 12), 30) != 30) return false;
  config_file.close();
